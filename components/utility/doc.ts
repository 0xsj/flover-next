 ...
 *
 * `icon.ts` is the only file that may import `lucide-react`, so the export list
 ...
 *
 * It works by CLONING the child, so an icon that does not spread props onto its
 * element silently drops both attributes and ends up announced twice.
 *
 ...
 *
 * # Portal moves the DOM parent and not the React one
 *
 * Context still reaches, and a React event still bubbles to the component that
 * rendered it, while a native listener on the old DOM parent stops firing.
