...
export function rangeAvailabilityError(range: ReturnType<typeof calendarRange>, isDateUnavailable?: DateControlProps["isDateUnavailable"]) {
...
export function dateConstraints({ minDate, maxDate, isDateUnavailable }: DateControlProps) {
  ...
  if (minValue && maxValue && minValue.compare(maxValue) > 0) throw new RangeError("Minimum date must not follow maximum date.");
  return { minValue, maxValue, isDateUnavailable: isDateUnavailable ? (date: { toString(): string }) => isDateUnavailable(date.toString()) : undefined };
}
