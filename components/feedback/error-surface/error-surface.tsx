...
import { cn, isRetryable, type Failure } from "@/lib/kernel";
...
export function ErrorSurface({
  failure, digest, title, onRetry, action, className,
}: ErrorSurfaceProps) {
  const redacted = Boolean(digest);
  const worthRetrying = redacted || isRetryable(failure);

  return (
    <div className={cn(s.surface, className)} role="alert">
      <h2 className={s.title}>{title ?? "This did not load"}</h2>
      ...
      <dl className={s.refs}>
        {digest ? <Ref label="digest" value={digest} /> : null}
        {failure.correlationId ? <Ref label="interaction" value={failure.correlationId} /> : null}
        {failure.requestId ? <Ref label="request" value={failure.requestId} /> : null}
        ...
        {!redacted ? <Ref label="kind" value={failure.kind} /> : null}
      </dl>
...
function Ref({ label, value }: { label: string; value: string }) {
  return (
    <div className={s.ref}>
      <dt className={s.refLabel}>{label}</dt>
      <dd className={s.refValue}>{value}</dd>
    </div>
  );
}
