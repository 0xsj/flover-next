...
export type SpaceProps = {
  ...
  gap?: Space; gapX?: Space; gapY?: Space;
};
...
const value = (v: Space | Margin | undefined): string | undefined =>
  v === undefined ? undefined : v === "auto" ? "auto" : v === 0 ? "0" : `var(--space-${v})`;
...
export function spaceStyle(props: SpaceProps): CSSProperties {
  ...
  const style: CSSProperties = {
    ...
    paddingInlineStart: v(props.pl), paddingInlineEnd: v(props.pr),
    margin: v(props.m),
    marginInline: v(props.mx), marginBlock: v(props.my),
    marginBlockStart: v(props.mt), marginBlockEnd: v(props.mb),
...
export function splitSpace<T extends SpaceProps>(
  props: T,
): [CSSProperties, Omit<T, keyof SpaceProps>] {
  const rest = { ...props } as Record<string, unknown>;
  for (const k of SPACE_KEYS) delete rest[k];
