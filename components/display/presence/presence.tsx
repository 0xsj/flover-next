...
import { assertNever, type Presence as PresenceValue } from "@/lib/kernel";
...
export function Presence<T>({ of, children, className }: PresenceProps<T>) {
  switch (of.state) {
    case "found":
      return <>{children(of.value)}</>;
    case "empty":
      return (
        <span className={cn(s.absent, className)} title={PRESENCE_MEANING.empty}>
          {PRESENCE_WORD.empty}
        </span>
      );
    case "unmeasured":
      return (
        <span className={cn(s.unmeasured, className)} title={PRESENCE_MEANING.unmeasured}>
          {PRESENCE_WORD.unmeasured}
        </span>
      );
    default:
      return assertNever(of, "presence state");
  }
}
