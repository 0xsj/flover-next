 ...
 */
export function cose(input: GraphInput, options?: { seed?: number; quality?: "draft" | "default" | "proof" }): Placement {
  const { nodes, edges, width, height } = input;
  if (!nodes.length) return new Map();
  const ids = new Set(nodes.map((node) => node.id));
  const used = new Set(ids);
  const connections = edges.filter((edge) => ids.has(edge.from) && ids.has(edge.to)).map((edge, index) => {
    let id = `edge-${index}`;
  ...
  const cy = cytoscape({
    ...
    elements: [...nodes.map((node) => ({ data: { id: node.id } })), ...connections],
  });
  ...
  try {
    const layout = cy.layout({
      ...
      nodeOverlap: 12, idealEdgeLength: () => 70, componentSpacing: 60,
    ...
    layout.run();
    if (!stopped) throw new Error("CoSE must settle synchronously for scoped deterministic randomness.");
    ...
    const points = cy.nodes().map((node) => ({ id: node.id(), ...node.position() }));
    const minX = Math.min(...points.map((point) => point.x));
    const maxX = Math.max(...points.map((point) => point.x));
    const minY = Math.min(...points.map((point) => point.y));
    const maxY = Math.max(...points.map((point) => point.y));
    const scale = Math.min(Math.max(1, width - 64) / Math.max(1, maxX - minX), Math.max(1, height - 64) / Math.max(1, maxY - minY));
    const out: Placement = new Map(points.map((point) => [point.id, {
      x: px(width / 2 + (point.x - (minX + maxX) / 2) * scale),
      y: px(height / 2 + (point.y - (minY + maxY) / 2) * scale),
    }]));
    if (input.pins) for (const [id, at] of input.pins) if (ids.has(id)) out.set(id, at);
    return out;
  } finally {
    Math.random = real;
    cy.destroy();
