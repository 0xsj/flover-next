...
import { circular, columns, force, hopsFrom, tiered, type GraphInput } from "./layout";
import { cose } from "./layout-cose";
import { linear } from "./scale";

const graph: GraphInput = {
  nodes: [{ id: "a", group: "first" }, { id: "b", group: "second" }, { id: "c", group: "third" }],
  edges: [{ from: "a", to: "b" }, { from: "b", to: "c" }], width: 480, height: 320,
};
describe("chart geometry contracts", () => {
  ...
  it("does not recycle categorical identities or accept fractional slots", () => {
    ...
    for (const index of [-1, 0.5, 4, Infinity]) expect(categorical(index)).toBeNull();
  });
  ...
  it("produces stable force positions and lets a supplied pin win", () => {
    ...
    const at = { x: 111, y: 22 };
    expect(force({ ...graph, pins: new Map([["a", at]]) }).get("a")).toEqual(at);
    expect(circular({ ...graph, pins: new Map([["a", at]]) }).get("a")).toEqual(at);
  });
  ...
  it("walks graph distance in both directions", () => {
    expect([...hopsFrom(graph, "c")]).toEqual([["c", 0], ["b", 1], ["a", 2]]);
  });
  it("orders flow dependencies, while cycles still stay inside the layout", () => {
    ...
    const cyclic = columns({ ...graph, edges: [...graph.edges, { from: "c", to: "a" }] });
    for (const point of cyclic.values()) { expect(point.x).toBeGreaterThanOrEqual(0); expect(point.x).toBeLessThanOrEqual(graph.width); }
  });
  it("fits deterministic CoSE output and restores the global random source", () => {
    ...
    for (const point of first.values()) {
      ...
      expect(point.x).toBeGreaterThanOrEqual(0); expect(point.x).toBeLessThanOrEqual(graph.width);
      expect(point.y).toBeGreaterThanOrEqual(0); expect(point.y).toBeLessThanOrEqual(graph.height);
    }
  });
  it("handles empty, single, dangling, and edge-like node identifiers", () => {
    expect(cose({ ...graph, nodes: [], edges: [] }).size).toBe(0);
    expect(cose({ ...graph, nodes: [{ id: "only" }], edges: [] }).get("only")).toEqual({ x: 240, y: 160 });
    const result = cose({ ...graph, nodes: [{ id: "edge-0" }, { id: "other" }], edges: [{ from: "edge-0", to: "other" }, { from: "ghost", to: "other" }] });
