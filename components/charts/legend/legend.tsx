...
export function Legend({ items, className }: { items: readonly LegendItem[]; className?: string }) {
  ...
  return (
    <ul className={[s.legend, className].filter(Boolean).join(" ")} aria-label="Chart categories">
      {items.map((item) => (
        <li key={item.label}>
...
export function ColourBar({
  ...
  const cells = Array.from({ length: steps }, (_, i) => {
    const value = domain[0] + i / (steps - 1) * (domain[1] - domain[0]);
    const t = kind === "diverging" ? (value - midpoint) / extent : i / (steps - 1);
  ...
  return (
    <figure className={s.bar}>
      <svg viewBox={`0 0 ${px(width)} 26`} className={s.barSvg} style={{ inlineSize: width }} aria-hidden="true">
        {cells.map((c, i) => (
          <rect key={i} x={px(i * cellWidth)} y={0} width={px(cellWidth)} height={10} style={{ fill: c.fill }} />
        ))}
        <text className={s.barTick} x={0} y={23}>{fmt(domain[0])}</text>
        {kind === "diverging" ? (
          <text className={s.barTick} x={px(domain[1] === domain[0] ? width / 2 : (midpoint - domain[0]) / (domain[1] - domain[0]) * width)} y={23} textAnchor="middle">{fmt(midpoint)}</text>
        ) : null}
        <text className={s.barTick} x={px(width)} y={23} textAnchor="end">{fmt(domain[1])}</text>
      </svg>
