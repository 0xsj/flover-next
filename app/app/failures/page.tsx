...
export default async function FailuresPage({
  ...
  /* Typed by hand rather than with `PageProps<"/app/failures">`: the generated
      ...
      <Flex direction="column" gap={7}>
        ...
        <Panel title="Force one">
          <Flex direction="column" gap={5}>
            <div>
              ...
              <Flex gap={3} wrap>
                {TRANSPORT_KINDS.map((kind) => (
                  <Link key={kind} href={`/app/failures${PLAN(kind)}`} className={s.chip} data-on={forced?.includes(`:${kind}`) || undefined}>
                    {kind}
                  </Link>
                ))}
              </Flex>
            ...
            <Flex gap={4} wrap>
              <Link href="/app/failures" className={s.chip}>none — succeed</Link>
              <Link href="/app/failures?chaos=GET /auth/sessions=empty:list" className={s.chip}>empty:list</Link>
              <Link href="/app/failures?chaos=GET /auth/sessions=latency:3000" className={s.chip}>latency:3000</Link>
            </Flex>
          </Flex>
        </Panel>
          ...
          <Table>
            ...
            <TBody>
              {FAILURE_KINDS.map((kind) => (
                <Tr key={kind}>
                  <Td><Link href={`/app/failures${PLAN(kind)}`} className={s.mono}>{kind}</Link></Td>
                  <Td>
