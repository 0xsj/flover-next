 ...
 *
 *  The rail is derived from the rendered page and not declared beside it. The
 *  registry already gives sections that property — a section cannot exist and
 *  be unreachable — and a second hand-kept list of every case would have exactly
 *  the drift the registry was built to avoid.
 *
 *  `getSnapshot` returns a stable STRING, not an array. A fresh array on every
 *  call is a fresh identity on every call, which `useSyncExternalStore` reads as
 *  a change, which re-renders, which allocates another array — an infinite loop
 *  reported as "Maximum update depth exceeded". The entries are derived from
 *  that key with `useMemo`. */
...
export function CaseNav() {
  ...
  return (
    <nav className={s.caseNav} aria-label="Cases on this page">
      {entries.map((entry) => (
