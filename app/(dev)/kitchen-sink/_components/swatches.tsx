...
function useResolved(tokens: string[]): Record<string, string> {
  ...
  useEffect(() => {
    ...
    const read = () => {
      ...
      setValues(
        Object.fromEntries(names.map((t) => [t, style.getPropertyValue(t).trim()])),
      );
    };
    read();
...
export function Swatches({ ramps }: { ramps: Ramp[] }) {
  const values = useResolved(ramps.flatMap((r) => r.tokens));

  return (
    <>
      {ramps.map((ramp) => (
        <div key={ramp.name} className={s.ramp}>
          {ramp.name ? <div className={s.rampName}>{ramp.name}</div> : null}
          <div className={s.grid}>
            {ramp.tokens.map((token) => {
              const value = values[token];
              const missing = value === "";
              return (
                <div key={token} className={s.cell}>
                  {ramp.kind === "shadow" ? (
                    <div className={s.shadowChip} style={{ boxShadow: `var(${token})` }} />
                  ) : (
                    <div className={s.chip}>
                      <div className={s.paint} style={{ background: `var(${token})` }} />
                    </div>
                  )}
                  <div className={s.meta}>
                    <span className={s.token}>{token}</span>
                    <span className={missing ? s.unresolved : s.value}>
