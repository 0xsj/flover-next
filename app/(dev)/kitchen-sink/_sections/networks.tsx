import { BipartiteNetwork, CircularNetwork, CliqueNetwork, CorrelationNetwork, EnrichmentMap, FlowNetwork, ForceNetwork, HairballNetwork, Legend, ModuleNetwork, MultipartiteNetwork, RadialNetwork, GraphFrame, categorical } from "@/components/charts";
import { Text } from "@/components/typography";
import { Case, Section } from "../_components/section";
import { readSources } from "../_lib/source";
import { net, star, bipartite, correlation } from "./chart-fixtures";
import { NetworkSelectionDemo } from "./chart-selection-demo";

export async function NetworksSection() {
  const sources = await readSources(["charts/network-presets/network-presets.tsx", "charts/graph-frame/graph-frame.tsx", "charts/_kernel/layout-cose.ts"]);
  return <Section id="networks" title="Network diagrams" blurb="Eleven presets from Overwatch over one graph frame. Layout communicates structure; shapes, signed edges, and labelled group annotations add meaning. These are SVG diagrams with exact records alongside them.">
    <Case title="Network selection" note="inspect a node and its neighbors without changing the layout"><NetworkSelectionDemo /></Case>
    ...
    <Case title="Small and empty networks" note="a single node and an empty graph remain valid inputs"><GraphFrame title="Single node" nodes={[{ id: "solo", label: "Only node" }]} edges={[]} width={320} height={180} layout="circular" /><GraphFrame title="Empty graph" nodes={[]} edges={[]} width={320} height={180} /></Case>
  </Section>;
}
