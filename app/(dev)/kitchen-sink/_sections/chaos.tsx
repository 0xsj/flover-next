...
import { parsePlan, withChaos } from "@/lib/chaos";
import { Case, Row, Section } from "../_components/section";
import { demoClient, listRows, type Row as DemoRow } from "../_lib/demo";
...
const SCENARIOS = [
  ...
  { label: "empty",       query: "chaos=GET /demo=empty:list",             note: "a SUCCESS carrying nothing" },
  { label: "slow",        query: "chaos=GET /demo=latency:1500",           note: "the loading state, held open" },
  { label: "forbidden",   query: "chaos=GET /demo=fail:forbidden",         note: "nobody looked" },
  { label: "rate limited", query: "chaos=GET /demo=fail:rate_limited",     note: "carries the server's retry-after" },
  { label: "hang",        query: "chaos=GET /demo=hang",                   note: "never settles — the stuck spinner" },
  { label: "flaky",       query: "chaos=GET /demo=fail:internal,p:0.5&chaosSeed=7", note: "seeded, so it replays" },
] as const;

type View =
  | { state: "loading" }
  | { state: "found"; rows: DemoRow[] }
  | { state: "empty" }
...
export function ChaosSection() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      // The wrapper takes the plan; everything below it is the ordinary client.
      const client = withChaos(demoClient(), parsePlan(query));
      const result = await listRows(client);
  ...
  return (
    <Section
      id="chaos"
      title="Chaos"
      blurb="Not a component, so this is a harness rather than a specimen. Because the transport port returns failures as values, forcing one is a decorator over the client — no patched fetch, and everything above it is untouched. What it exists for is the half nobody tests: a screen's empty and loading branches, which the fixtures themselves hide."
    >
      <Case title="The four states, on demand" note="each button sets a plan and re-runs the same request">
        {!enabled ? (
      ...
      <Case title="A plan is a link" note="so a broken state is something you can send somebody">
