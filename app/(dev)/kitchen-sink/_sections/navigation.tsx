import {
  Breadcrumb, BreadcrumbCurrent, BreadcrumbLink, NavLink,
...
export function NavigationSection() {
  return (
    ...
    >
      <Case title="NavLink" note="active is a prop; the component does not read the route">
        <Box className={s.navWidth}>
      ...
      <Case title="Breadcrumb" note="the last crumb is not a link">
        ...
        <p className={s.limits}>
          ...
          span carrying <code>aria-current</code> — the meaning was never in the
          anchor. Separators are real elements with <code>aria-hidden</code>
          rather than generated content, which some readers announce and others
          do not.
        </p>
      </Case>

      <Case title="Tabs" note="automatic activation, until showing a panel costs something">
        <Row label="automatic">
          <Box className={s.fullWidth}>
            <Tabs defaultValue="overview">
              ...
              <TabPanel value="overview">Arrowing here showed this panel immediately.</TabPanel>
        ...
        <p className={s.limits}>
          Automatic is the platform contract and what a keyboard user expects, so
          it is the default. <strong>Switch to manual when showing a panel is
          expensive</strong> — a fetch, a chart — because travelling past four
          tabs then costs one panel rather than four. Right default, wrong one
          for a dashboard.
        </p>
        <p className={s.limits}>
