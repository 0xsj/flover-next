import { AccessibleIcon, Check, ChevronDown, LoaderCircle, Minus, Plus, Trash2, VisuallyHidden, X } from "@/components/utility";
...
export function UtilitySection() {
  return (
    ...
    >
      <Case title="Icon" note="one file of named re-exports — the inventory, not a guess">
      ...
      <Case title="AccessibleIcon" note="the rarest of the three ways to name an icon, not the default">
        <Row label="decorative">
          <DomReadout note="The icon repeats what the label already says. aria-hidden, no component, and this is what the design system does in eight places.">
            <Button intent="primary"><Trash2 size={14} aria-hidden="true" />Delete target</Button>
          </DomReadout>
        </Row>
        <Row label="icon-only control">
          <DomReadout note="The name belongs on the CONTROL. Naming the icon as well announces it twice, and that mistake reads as extra care, which is why it survives review.">
            <Button intent="danger" aria-label="Delete target"><Trash2 size={14} aria-hidden="true" /></Button>
          </DomReadout>
        </Row>
        <Row label="AccessibleIcon">
          <DomReadout note="No labellable ancestor — a status glyph in a cell. The icon is hidden twice over, and the name is a clipped span beside it.">
            <span className={s.iconCell}>
              <AccessibleIcon label="Passing"><Check size={16} /></AccessibleIcon>
            </span>
          </DomReadout>
        </Row>
        <p className={s.limits}>
          <code>focusable=&quot;false&quot;</code> is not decoration:{" "}
          <code>aria-hidden</code> removes an SVG from the accessibility tree and
          does not remove it from the tab order. It works by{" "}
          <strong>cloning</strong> the child, so an icon that does not spread its
          props swallows both attributes and ends up announced twice — through
          the component meant to prevent exactly that.
        </p>
      </Case>

      <Case title="VisuallyHidden" note="text for a reader and not for the screen">
        <Row label="in a sentence">
          <DomReadout note="The screen shows five words; a reader gets nine. The clipped fragment is the difference, and the markup below is where it lives.">
            <span>
              Delete
              <VisuallyHidden> this workspace and everything in it</VisuallyHidden>
            </span>
          </DomReadout>
        </Row>
        <Row label="asChild">
          <DomReadout note="Hides an element the caller already owns rather than nesting one inside a hidden span. The dialog's hideTitle uses this, so the primitive still sees its own Title element.">
            <VisuallyHidden asChild><h4>Filters</h4></VisuallyHidden>
          </DomReadout>
        </Row>
        <p className={s.limits}>
      ...
      <Case title="Portal" note="the DOM parent moves; the React parent does not">
        <PortalCase />
        <p className={s.limits}>
          Reach for it only to escape an ancestor that clips or contains —{" "}
          <code>overflow: hidden</code>, or a <code>transform</code>, which makes
          that ancestor the containing block for everything fixed inside it.
          Every overlay in this system already portals itself, so this is rarely
          the answer.
        </p>
        <p className={s.limits}>
          The counter is on the clipping box. It keeps incrementing after the
          content leaves, because a React event follows the{" "}
          <strong>React</strong> tree — a handler that is no longer a DOM
