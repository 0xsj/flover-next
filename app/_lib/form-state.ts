...
export type ToFormStateOptions = {
   ...
   *
   *  Needed because React resets an uncontrolled form after its action
   *  completes — including when the action refused — so the email a reader just
   *  typed disappears at the moment they are told to try again.
   *
   *  Never a password. */
  values?: Record<string, string>;
...
export function toFormState(failure: Failure, options: ToFormStateOptions = {}): FormState {
  const base = { status: "error", kind: failure.kind, values: options.values } as const;
  ...
  if (failure.kind === "invalid") {
    return { ...base, scope: "fields", message: failure.message, fields: failure.fields };
  }

  const field = options.fieldFor?.(failure);
  if (field) {
    return { ...base, scope: "fields", message: failure.message, fields: { [field]: failure.message } };
  }

  switch (failure.kind) {
    case "rate_limited":
      return { ...base, scope: "form", message: failure.message, retryAfter: failure.retryAfter };
