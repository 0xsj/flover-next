...
export const metadata: Metadata = { title: "Cookbook · flover" };
    ...
    <div className={s.grid}>
      {RECIPES.map(({ href, label, description, icon: Icon, category }) => <Card key={href}>
        <CardHeader leading={<Icon size={20} aria-hidden="true" />}>
          <Text as="span" size="sm" tone="muted">{category}</Text>
          <CardTitle level={2}><CardLink asChild><Link href={href}>{label}</Link></CardLink></CardTitle>
          <CardDescription>{description}</CardDescription>
        </CardHeader>
      </Card>)}
    </div>
    <div className={s.footer}>
      <Text size="sm" tone="muted">The working examples use a signed-in session. You can sign in with the supplied demo account.</Text>
      <Text size="sm"><Link href="/kitchen-sink">Browse individual components</Link></Text>
    </div>
  </Flex></Container>;
}
