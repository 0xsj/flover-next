...
export function createLiveDemo() {
  ...
  return {
    source,
    emit() { revision++; source.publish({ type: "metrics.changed", revision }); },
    failReads(value: boolean) { failing = value; },
    async read() {
      reads++;
      return failing ? err(unavailable("The simulated data source is unavailable."))
        : ok({ requests: 1240 + revision * 17, queued: Math.max(0, 12 - revision), revision, reads });
    },
  };
}
