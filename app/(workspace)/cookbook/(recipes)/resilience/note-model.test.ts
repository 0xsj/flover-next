...
import { err, invalid, ok, timeout, unavailable, type Result } from "@/lib/kernel";
import { createNoteExample, createRaceExample, readResponseExample } from "@/lib/root/resilience";
...
import type { NoteAttempt, NoteReceipt } from "@/lib/services/example/note.types";
import { createNoteModel } from "./note-model";
import { createRaceModel } from "./race-model";

const draft = { title: "My title", body: "Every character stays.\nEven this line." };
function deferred<T>() {
...
describe("draft recovery guarantees", () => {
  it("preserves the draft after a definite refusal and permits a corrected save", async () => {
    ...
    await model.save();
    expect(model.get()).toMatchObject({ draft, confirmed: null, phase: { state: "refused" } });
    expect(example.committed()).toBe(0);
    example.setSaveMode("success");
    await model.save();
    expect(model.get()).toMatchObject({ draft, confirmed: { draft, revision: 1 }, phase: { state: "ready" } });
  });
  ...
  it("lost acknowledgement commits once, blocks further writes, and reconciles without writing", async () => {
    ...
    const model = createNoteModel({ ...example, save }, draft, () => "original-operation");
    await model.save();
    expect(example.committed()).toBe(1);
    expect(model.get()).toMatchObject({ draft, confirmed: null, phase: { state: "unknown", attempt: { operationId: "original-operation", draft } } });
    await model.save();
  ...
  it("failed reconciliation remains unknown and retains the original operation", async () => {
    ...
    await model.check();
    expect(model.get()).toMatchObject({ draft, confirmed: null, phase: { state: "unknown", attempt: { operationId: "original" }, failure: { kind: "unavailable" } } });
  ...
  it("prevents overlapping saves and does not overwrite edits made while saving", async () => {
    ...
    const model = createNoteModel({ save, find: async () => ok(null) }, draft, () => "captured");
    ...
    await first;
    expect(save).toHaveBeenCalledTimes(1);
    expect(model.get()).toMatchObject({ draft: { title: "Newer title" }, confirmed: { draft }, phase: { state: "ready" } });
  });
  ...
  it("allows editing during a check and deduplicates repeated check activations", async () => {
    ...
    const model = createNoteModel({ save: async () => err(timeout("lost")), find }, draft, () => "captured");
    await model.save();
    ...
    expect(model.get()).toMatchObject({ draft: { body: "Newer body" }, confirmed: { draft } });
  });

  it("does not mistake a malformed or unrelated receipt for confirmation", async () => {
    const model = createNoteModel({ save: async () => err(timeout("lost")), find: async () => ok({ operationId: "different", draft, revision: 1 }) }, draft, () => "original");
    await model.save();
    await model.check();
    expect(model.get()).toMatchObject({ confirmed: null, phase: { state: "unknown", failure: { type: "invalid_response" } } });
  });

  it("cleanup aborts work without treating an interrupted write as a definite refusal", async () => {
    const pending = deferred<Result<NoteReceipt>>();
    let signal: AbortSignal | undefined;
    const model = createNoteModel({ save: (_attempt, active) => { signal = active; return pending.promise; }, find: async () => err(unavailable("offline")) }, draft, () => "original");
    const active = model.save();
    model.cancel();
    expect(signal?.aborted).toBe(true);
    pending.resolve(ok({ operationId: "original", draft, revision: 1 }));
    await active;
    expect(model.get()).toMatchObject({ draft, confirmed: null, phase: { state: "unknown", attempt: { operationId: "original" } } });
  });
...
describe("the example backend's explicit receipt contract", () => {
  it("deduplicates an identical operation and refuses reuse with different content", async () => {
    ...
    const conflict = await saveNote(client, { ...attempt, draft: { ...draft, title: "different" } });
    expect(!conflict.ok && conflict.error.kind).toBe("conflict");
    expect((await findNoteSave(client, "same")).unwrapOr(null)).toEqual(first.unwrapOr(null));
  });

  it("an arbitrary lookup refusal is not evidence of absence", async () => {
    const client = createMemoryClient({ latencyMs: 0, routes: [{ method: "GET", pattern: /.*/, handle: () => err(invalid("bad request", {})) }] });
...
it("replays the real out-of-order simulation without accepting the earlier result", async () => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await model.release();
    expect(model.reader.get()).toMatchObject({ state: "ready", value: [{ id: "newer" }] });
...
it("the malformed cookbook cases use the actual service decoder", async () => {
  for (const mode of ["malformed", "partial"] as const) {
    const result = await readResponseExample(mode, new AbortController().signal);
