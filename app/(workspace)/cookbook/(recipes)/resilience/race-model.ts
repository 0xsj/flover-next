...
type RaceSource = {
  read: (selection: "earlier" | "newer", signal: AbortSignal) => Promise<Result<Item[]>>;
  release: () => void;
  dispose: () => void;
};

export function createRaceModel(createSource: () => RaceSource) {
  const phase = createStore<"idle" | "running" | "waiting" | "complete">("idle");
  const reader = createLatestRead((input: { source: RaceSource; selection: "earlier" | "newer" }, signal) => input.source.read(input.selection, signal));
