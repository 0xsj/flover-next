...
function blockMarkdown(block: ManualBlock, origin: string): string {
  switch (block.kind) {
    ...
    case "links": return block.items.map(item => `- [${item.label}](${new URL(item.href, origin).href}) — ${item.description}`).join("\n");
    case "sources": return `Repository references:\n\n${block.items.map(item => `- \`${item.path}\` — ${item.why}`).join("\n")}`;
...
export function manualMarkdown(origin: string): string {
  return [
    ...
    ...chapters.map(chapter => [
      ...
      ...chapter.sections.map(section => [`### ${section.title}`, ...section.blocks.map(block => blockMarkdown(block, origin))].join("\n\n")),
    ].join("\n\n")),
  ].join("\n\n") + "\n";
}
