...
describe("12 · the transport port", () => {
  ...
  describe("a failure with no response", () => {
    ...
    it("maps exceeding its own time budget to timeout — distinct from cancellation, and retryable", async () => {
  ...
  describe("the in-memory adapter", () => {
    ...
    it("forwards the request body to the matching route's handler", async () => {
      ...
      const result = await client.post("/widgets", { name: "gadget" });
  ...
  describe("requireToken", () => {
    it("returns the token carried on a request that has one", () => {
