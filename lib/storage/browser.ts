import { err, forbidden, internal, ok, conflict, unavailable, type Failure, type Result } from "../kernel";
...
function failure(error: unknown): Failure {
  ...
  if (name === "SecurityError") return forbidden("Browser storage is blocked.", { type: "storage_blocked" });
  if (name === "QuotaExceededError") return conflict("Browser storage is full. The change was not saved.", { type: "storage_quota" });
...
export function createBrowserStorage(area: "local" | "session" = "local"): StoragePort {
  ...
  function change(key: string, operation: (storage: Storage) => void): Result<void> {
    const result = access(storage => { operation(storage); return storage; });
  ...
  return {
    ...
    subscribe(key, listener) {
      return access(storage => {
        const local = (changed: string) => { if (changed === key) listener(); };
        const remote = (event: StorageEvent) => {
          if (event.storageArea === storage && (event.key === key || event.key === null)) listener();
        };
        ...
        return () => { group.delete(local); window.removeEventListener("storage", remote); };
      });
    },
  };
}
