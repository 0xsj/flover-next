...
function envelope(raw: string): Result<Envelope> {
  try {
    ...
    if (typeof data !== "object" || data === null || !("version" in data) || !("value" in data)
      || !Number.isSafeInteger(data.version) || (data.version as number) < 1) return err(malformed());
    return ok({ version: data.version as number, value: data.value });
  } catch { return err(malformed()); }
}

/** No `read<T>(key)` assertion: every read proves the registered schema. */
export function createDocument<T>(port: StoragePort, namespace: string, schema: DocumentSchema<T>): StoredDocument<T> {
  const key = `${encodeURIComponent(namespace)}:${encodeURIComponent(schema.key)}`;
  const future = () => conflict("This document was saved by a newer version. Reset it explicitly before replacing it.", { type: "storage_version" });
  function decode(value: unknown): Result<T> {
    try { return schema.decode(value); } catch { return err(malformed()); }
  }
  function read(): Result<Stored<T>> {
    ...
    let value: Result<T>;
    if (migrated) {
      if (!schema.migrate) return err(conflict("This saved version needs a migration or an explicit reset.", { type: "storage_version" }));
      try { value = schema.migrate(saved.value.value, saved.value.version).andThen(decode); }
      catch { return err(malformed()); }
