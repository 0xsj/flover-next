...
export function createMemoryStorage(): StoragePort {
  ...
  return {
    ...
    write(key, value) { values.set(key, value); notify(key); return ok(undefined); },
    remove(key) { values.delete(key); notify(key); return ok(undefined); },
    subscribe(key, listener) {
      const group = listeners.get(key) ?? new Set<() => void>();
      listeners.set(key, group);
      group.add(listener);
      return ok(() => { group.delete(listener); });
    },
  };
}
