...
const FAILURES: Record<FailureKind, (m: string) => Failure> = {
  unauthenticated, forbidden, not_found: notFound, invalid: (m) => invalid(m, {}),
  conflict, rate_limited: (m) => rateLimited(m, 12), unavailable, timeout,
  canceled, internal,
};
    ...
    if (effect.hang) {
      ...
      return new Promise<Result<T, Failure>>((resolve) => {
        options.signal?.addEventListener(
          "abort", () => resolve(err(canceled("The request was cancelled."))), { once: true },
        );
      });
    }
