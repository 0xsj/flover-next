...
export function createSequenceClient(inner: HttpClient, steps: readonly SequenceStep[]) {
  ...
  const hold = (gate: string, signal: AbortSignal) => new Promise<void>(resolve => {
    if (signal.aborted || released.has(gate)) { resolve(); return; }
  ...
  const request = async <T>(method: string, path: string, options: RequestOptions = {}): Promise<Result<T, Failure>> => {
    ...
    const signal = ignoreAbort || !options.signal ? lifecycle.signal : AbortSignal.any([lifecycle.signal, options.signal]);
  ...
  const client: HttpClient = {
    request,
    get: (p, o) => request("GET", p, o), post: (p, o) => request("POST", p, o),
