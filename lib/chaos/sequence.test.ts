...
function fixture() {
  ...
  const client = createMemoryClient({ latencyMs: 0, routes: [
    { method: "PUT", pattern: /^\/note$/, handle: () => ok({ revision: ++writes }) },
...
describe("scripted request sequences", () => {
  it("consumes only the next matching step, exactly once, in invocation order", async () => {
    ...
    const [one, two] = await Promise.all([script.client.get("/a"), script.client.get("/a")]);
  ...
  it("does not replace a genuine refusal with a lost-success effect", async () => {
    const client = createMemoryClient({ latencyMs: 0, routes: [{ method: "PUT", pattern: /.*/, handle: () => err(notFound("gone")) }] });
    const script = createSequenceClient(client, [{ request: "*", effect: { kind: "lose-response", failure: timeout("lost") } }]);
  ...
  it("delivers newer data first and the original captured response only after release", async () => {
    const script = createSequenceClient(fixture().client, [{ request: "GET /a", effect: { kind: "hold-response", gate: "a", ignoreAbort: true } }]);
    const abort = new AbortController();
    let finished = false;
    const old = script.client.get("/a", { signal: abort.signal }).then(result => { finished = true; return result; });
    abort.abort();
    expect((await script.client.get("/b")).unwrapOr(null)).toBe("/b");
  ...
  it("disposal releases even uncooperative work and prevents new writes", async () => {
    const server = fixture();
    const script = createSequenceClient(server.client, [{ request: "*", effect: { kind: "hold-response", gate: "a", ignoreAbort: true } }]);
  ...
  it("a pre-aborted request does not consume a step", async () => {
    const script = createSequenceClient(fixture().client, [{ request: "*", effect: { kind: "reply", value: "scripted" } }]);
    expect((await script.client.get("/a", { signal: AbortSignal.abort() })).ok).toBe(false);
