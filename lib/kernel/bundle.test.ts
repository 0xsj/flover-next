...
describe("no fixture reaches the browser bundle", () => {
  ...
  it("the needles are real — each one exists in a fixture", async () => {
    ...
    const sources = await Promise.all([
      readFile(path.join(process.cwd(), "lib/services/ledger/ledger.fixtures.ts"), "utf8"),
      readFile(path.join(process.cwd(), "lib/services/session/session.fixtures.ts"), "utf8"),
    ]);
