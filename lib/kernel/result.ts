 ...
 * as in usage/3-server-action.tsx. */

export class Ok<T, E = Failure> {
  readonly ok = true as const;
  constructor(readonly value: T) {}

  map<U>(f: (value: T) => U): Result<U, E> { return new Ok(f(this.value)); }
  ...
  toJSON() { return { ok: true as const, value: this.value }; }
}
...
export class Err<T, E = Failure> {
  ...
  map<U>(_f: (value: T) => U): Result<U, E> { return new Err(this.error); }
