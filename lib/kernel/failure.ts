...
export const TRANSPORT_KINDS = [
  "unauthenticated", "forbidden", "rate_limited", "unavailable", "timeout", "canceled", "internal",
] as const;
...
export const invalid = (message: string, fields: Record<string, string>, m: MetaInit = {}): Fails<"invalid"> =>
  ({ kind: "invalid", message, fields, ...m });
export const conflict = (message: string, m: MetaInit = {}): Fails<"conflict"> =>
  ({ kind: "conflict", message, ...m });

/* ── classification ─────────────────────────────────────────────────────── */

export function isFailureKind(v: unknown): v is FailureKind {
  return typeof v === "string" && (FAILURE_KINDS as readonly string[]).includes(v);
}
export function isTransportKind(v: unknown): v is TransportKind {
  return typeof v === "string" && (TRANSPORT_KINDS as readonly string[]).includes(v);
}
export function isDomainKind(v: unknown): v is DomainKind {
  return typeof v === "string" && (DOMAIN_KINDS as readonly string[]).includes(v);
}
...
export const isTransport = (f: Failure): f is TransportFailure => isTransportKind(f.kind);
...
export function narrow<K extends DomainKind>(
  ...
  return (f) => {
    ...
    return because(
      internal(f.message, {
        type: f.type, requestId: f.requestId, correlationId: f.correlationId, status: f.status,
      }),
      f,
    );
  };
...
export function isRetryable(f: Failure): boolean {
  return f.kind === "rate_limited" || f.kind === "unavailable" || f.kind === "timeout";
}
...
export function retryDelay(f: Failure, attempt: number): number | null {
  ...
  if (f.kind === "rate_limited" && f.retryAfter !== undefined) return f.retryAfter * 1000;
