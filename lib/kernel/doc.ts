 ...
 *   4. `Result` carries the failure, so nothing below the React edge throws.
 *
 * # Why `Result` is a class here and `Failure` is not
 *
 * A `Failure` crosses the RSC boundary constantly — it is passed as props and
 ...
 * # The floor
 *
 * This package imports nothing — not React, not a component, not `lib/http`.
