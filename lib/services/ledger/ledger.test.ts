...
import { sessionRoutes, signIn, DEMO_CREDENTIALS, resetSessionFixtures } from "../session";
...
describe("facets", () => {
  it("arrive with the FIRST page and not with later ones", async () => {
    const first = await getMyActivity(client());
    if (!first.ok || !first.value.next) return expect.unreachable("expected a next page");
  ...
  it("count the WHOLE log, not the filtered set", async () => {
    ...
    if (!all.ok || !filtered.ok) return expect.unreachable("the reads should have succeeded");
...
describe("the log is a real record of what happened", () => {
  it("signing in writes a row, and it names the interaction", async () => {
    ...
    if (after.ok) {
      expect(after.value.entries.map((e) => e.action)).toContain("session.signed_in");
      expect(after.value.entries.every((e) => e.correlation_id === "cid-test")).toBe(true);
    }
  });
  ...
  it("a REFUSED sign-in is recorded, and the actor is anonymous", async () => {
    ...
    if (!r.ok) return expect.unreachable("the read should have succeeded");
    const row = r.value.entries.find((e) => e.action === "session.sign_in_failed");
  ...
  it("detail is always an object, so a caller can index it without a guard", async () => {
    record({ scope: "system", action: "system.thing", subject: "x", actor: "system", correlationId: "cid-test" });
    const r = await getMyActivity(client(), { correlation: "cid-test" });
    expect(r.ok && r.value.entries.every((e) => e.detail && typeof e.detail === "object")).toBe(true);
  });

  it("a row with no correlation says so rather than inventing one", async () => {
    record({ scope: "system", action: "system.thing", subject: "x", actor: "system", correlationId: undefined });
