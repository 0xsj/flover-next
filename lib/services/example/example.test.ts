...
import { err, isRetryable, notFound, ok, presenceOf, rateLimited } from "../../kernel";
import { createMemoryClient, type MemoryRoute } from "../../http";
import {
  createItem, findDefaultItem, getItem, itemPage, listItems, renameItem,
  NO_DEFAULT, type Item,
} from ".";
...
const routes: MemoryRoute[] = [
  { method: "GET", pattern: /^\/items$/, handle: (req) =>
      req.params?.workspace === "empty" ? ok([]) : ok(items) },
  ...
  { method: "GET", pattern: /^\/items\/gone$/, handle: () => err(notFound("No such item.", { status: 404 })) },
  { method: "GET", pattern: /^\/items\/busy$/, handle: () => err(rateLimited("Slow down.", 12)) },
  { method: "GET", pattern: /^\/items\/moved$/, handle: () =>
      err({ kind: "conflict", message: "It moved.", status: 409 }) },
  { method: "GET", pattern: /^\/workspaces\/set\/default-item$/, handle: () => ok(items[0]) },
  { method: "GET", pattern: /^\/workspaces\/none\/default-item$/, handle: () =>
      err(notFound("No default.", { status: 404, type: NO_DEFAULT })) },
  { method: "GET", pattern: /^\/workspaces\/typo\/default-item$/, handle: () =>
      err(notFound("No such path.", { status: 404 })) },
  { method: "POST", pattern: /^\/items$/, handle: (req) =>
      (req.body as Item).name === "api"
        ? err({ kind: "conflict", message: "Name taken.", status: 409 })
        : ok({ ...(req.body as Item), id: "i3" }) },
  { method: "PATCH", pattern: /^\/items\/i1$/, handle: (req) => ok({ ...items[0], ...(req.body as Item) }) },
];
...
describe("absence is a value where the service says it is", () => {
  it("found", async () => {
    expect(presenceOf(await findDefaultItem(client, "set")).state).toBe("found");
  });
  ...
  it("all three states are distinct, which is the whole point", async () => {
    const states = await Promise.all(
      ["set", "none", "typo"].map(async (w) => presenceOf(await findDefaultItem(client, w)).state),
    );
...
describe("a write", () => {
  ...
  it("and surfaces the server's own refusal", async () => {
    const r = await createItem(client, { name: "api", host: "other.example.com" });
    if (r.ok) throw new Error("expected conflict");
    expect(r.error.kind).toBe("conflict");
  });

  it("succeeds otherwise", async () => {
    const r = await createItem(client, { name: "new", host: "new.example.com" });
