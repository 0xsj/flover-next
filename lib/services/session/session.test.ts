...
import {
  chain, isRetryable, rootCause,
  DOMAIN_KINDS, FAILURE_KINDS, TRANSPORT_KINDS,
} from "../../kernel";
import { parsePlan, withChaos } from "../../chaos";
import { createMemoryClient } from "../../http";
import {
  currentUser, listSessions, revokeSession, signIn, signOut, signUp,
} from "./session.api";
import { DEMO_CREDENTIALS, resetSessionFixtures, sessionRoutes } from "./session.fixtures";
...
const clientWith = (token: string | null = null) =>
  createMemoryClient({ routes: sessionRoutes, getAccessToken: () => token, latencyMs: 0 });
...
describe("signing in", () => {
  ...
  it("wrong password is UNAUTHENTICATED, not a field error", async () => {
    /* The distinction the whole service exists to demonstrate. `invalid` carries
       ...
       was wrong. */
    const r = await signIn(clientWith(), { ...DEMO_CREDENTIALS, password: "wrong-one" });
  ...
  it("an unknown account is refused identically to a wrong password", async () => {
    ...
    const unknown = await signIn(clientWith(), { email: "nobody@example.com", password: "password" });
    const wrong = await signIn(clientWith(), { ...DEMO_CREDENTIALS, password: "not-it" });
  ...
  it("a malformed form never reaches the transport", async () => {
    ...
    const r = await signIn(clientWith(), { email: "not-an-email", password: "" });
    expect(r.ok).toBe(false);
...
describe("signing up", () => {
  ...
  it("and the account can then sign in — the store outlives the request", async () => {
    ...
    expect((await signIn(clientWith(), { email: fresh.email, password: fresh.password })).ok).toBe(true);
  });

  it("a taken email is a CONFLICT, not an invalid field", async () => {
    // The form was well-formed; the world disagreed with it.
    const r = await signUp(clientWith(), { ...fresh, email: DEMO_CREDENTIALS.email });
    expect(r.ok).toBe(false);
  ...
  it("a padded, differently-cased email is the SAME account", async () => {
    ...
    const r = await signUp(clientWith(), { ...fresh, email: `  ${DEMO_CREDENTIALS.email.toUpperCase()} ` });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error.kind).toBe("conflict");
  });
  ...
  it("a short password is refused by the service before the wire", async () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok && r.error.kind === "invalid") expect(r.error.fields.password).toBeTruthy();
...
describe("everywhere this account is signed in", () => {
  ...
  it("never returns the token it is describing", async () => {
    ...
    if (!a.ok || !b.ok) return expect.unreachable("sign-in should have succeeded");
...
describe("it behaves like a server", () => {
  ...
  it("a cancellation resolves when it is asked for, not when the request would have finished", async () => {
    const slow = createMemoryClient({
      routes: sessionRoutes, getAccessToken: () => null, latencyMs: 5_000,
    });
...
describe("the contract holds under every forced kind", () => {
  ...
  const forced = (kind: string) =>
    withChaos(
      createMemoryClient({ routes: sessionRoutes, getAccessToken: () => "tok_x", latencyMs: 0 }),
      parsePlan(`?chaos=GET /auth/sessions=fail:${kind}`),
      "cid-test",
    );

  it.each([...TRANSPORT_KINDS])("a transport kind arrives unchanged: %s", async (kind) => {
  ...
  it.each([...DOMAIN_KINDS])("an undeclared domain kind folds, and keeps its cause: %s", async (kind) => {
    const r = await listSessions(forced(kind));
    expect(r.ok).toBe(false);
    if (!r.ok) {
