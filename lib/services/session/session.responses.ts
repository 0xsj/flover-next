...
function readUser(value: unknown): User | undefined {
  ...
  if (!user || !responseText(user.id) || !responseText(user.name) || !responseText(user.email)) return undefined;
...
function readSessionSummary(value: unknown): SessionSummary | undefined {
  const session = responseObject(value);
  if (!session || !responseText(session.id) || !responseTimestamp(session.createdAt) || typeof session.current !== "boolean") return undefined;
...
export const decodeSessions = responseDecoder("session list", responseArray(readSessionSummary));
