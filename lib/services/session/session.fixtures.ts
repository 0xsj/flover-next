import {
  conflict, err, invalid, notFound, ok, rateLimited, unauthenticated,
  type Failure, type Result,
} from "../../kernel";
import { requireToken, type MemoryRoute } from "../../http";
import { record } from "../ledger";
import {
  MIN_PASSWORD, type Session, type SessionSummary, type User,
} from "./session.types";
...
const issue = (user: StoredUser): Session => {
  ...
  sessions.set(token, {
    id: `ses_${newId()}`, userId: user.id, token, createdAt: Date.now(),
  });
...
export const sessionRoutes: MemoryRoute[] = [
  {
    ...
    handle: (req): Result<Session, Failure> => {
      ...
      if (!email || !password) {
        return err(invalid("Check the form.", {
      ...
      if ((failedAttempts.get(email) ?? 0) >= ATTEMPT_LIMIT) {
        return err(rateLimited("Too many attempts. Try again shortly.", RETRY_AFTER_SECONDS, { status: 429 }));
      }
      ...
      if (!user || user.password !== password) {
        ...
        record({
          scope: "session", action: "session.sign_in_failed",
          subject: email, actor: "anonymous",
          correlationId: req.correlationId, detail: { reason: "wrong_password", attempt },
        });
      ...
      record({
        scope: "session", action: "session.signed_in",
        subject: user.email, actor: user.email,
        correlationId: req.correlationId, detail: {},
      });
...

  {
    ...
    handle: (req): Result<Session, Failure> => {
      ...
      const user: StoredUser = { id: `u_${newId()}`, name, email: str(body.email).trim(), password };
      ...
      record({
        scope: "account", action: "account.created",
        subject: user.email, actor: "anonymous",
        correlationId: req.correlationId, detail: { via: "sign-up" },
      });
      record({
        scope: "session", action: "session.signed_in",
        subject: user.email, actor: user.email,
...

  {
    ...
    handle: (req): Result<null, Failure> => {
      ...
      if (ending) {
        ...
        record({
          scope: "session", action: "session.signed_out",
          subject: ending.id, actor: user?.email ?? "unknown",
          correlationId: req.correlationId, detail: {},
        });
      }
...

  {
    ...
    handle: (req, match): Result<null, Failure> => {
      ...
      record({
        scope: "session", action: "session.revoked",
        subject: target.id, actor: byId(mine.userId)?.email ?? "unknown",
        correlationId: req.correlationId, detail: { revoked_by: mine.id },
      });
      return ok(null);
    },
  },
