...
export function createMemorySource<T>(): EventSource<T> & {
  ...
  return {
    ...
    publish(value) { if (connection.state === "open") for (const observer of observers) observer.event(value); },
    disconnect: () => state({ state: "closed" }),
    reconnect: () => state({ state: "open", reconnected: true }),
    reject(failure) { for (const observer of observers) observer.error(failure); },
  };
}
