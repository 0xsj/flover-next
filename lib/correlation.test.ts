...
describe("the header is sent", () => {
  it("goes out on every request when an id is supplied", async () => {
    ...
    const client = createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => CID });
    await client.get("/a");
    ...
    for (const init of seen) {
      expect((init.headers as Record<string, string>)[CORRELATION_HEADER]).toBe(CID);
    }
  });
  ...
  it("sends nothing at all when the hook is absent or returns undefined", async () => {
    ...
    await createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => undefined }).get("/b");
    vi.unstubAllGlobals();
    for (const init of seen) {
  ...
  it("is read LAZILY, so a new interaction is picked up without a new client", async () => {
    ...
    let current = "first";
    const client = createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => current });
    await client.get("/a");
    current = "second";
    await client.get("/b");
    vi.unstubAllGlobals();
...
describe("a failure can always name its interaction", () => {
  const stub = (init: ResponseInit) =>
    vi.stubGlobal("fetch", async () => new Response(JSON.stringify({ message: "no" }), init));

  it("carries what we SENT when the server ignores the header", async () => {
    stub({ status: 500, headers: { "content-type": "application/json" } });
    const r = await createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => CID }).get("/a");
    vi.unstubAllGlobals();
  ...
  it("prefers the server's echo, because the server saw the request", async () => {
    stub({ status: 500, headers: { "content-type": "application/json", [CORRELATION_HEADER]: "server-said" } });
    const r = await createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => CID }).get("/a");
    vi.unstubAllGlobals();
  ...
  it("survives a transport failure, which is when it matters most", async () => {
    vi.stubGlobal("fetch", async () => { throw new Error("socket hung up"); });
    const r = await createFetchClient({ baseUrl: "https://api.test", getCorrelationId: () => CID }).get("/a");
    vi.unstubAllGlobals();
    if (!r.ok) { expect(r.error.kind).toBe("unavailable"); expect(r.error.correlationId).toBe(CID); }
  });
});

describe("the memory adapter is indistinguishable", () => {
  const routes: MemoryRoute[] = [
    { method: "GET", pattern: /^\/echo$/, handle: (req) => err(notFound(String(req.correlationId), { status: 404 })) },
    { method: "GET", pattern: /^\/plain$/, handle: () => err(notFound("gone", { status: 404 })) },
  ];
  const client = createMemoryClient({ routes, latencyMs: 0, getCorrelationId: () => CID });
  ...
  it("attaches it to an unserved route too", async () => {
    ...
    if (!r.ok) { expect(r.error.kind).toBe("internal"); expect(r.error.correlationId).toBe(CID); }
  });
});

describe("the thread is not cut by the folds", () => {
  const withCid: Failure = { kind: "conflict", message: "moved", correlationId: CID, status: 409 };
