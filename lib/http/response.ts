...
export type ResponseDecoder<T> = (value: unknown, trace?: DiagnosticTrace) => Result<T, Fails<"internal">>;
...
export function responseDecoder<T>(contract: string, read: ResponseReader<T>): ResponseDecoder<T> {
  const decode = (value: unknown): Result<T, Fails<"internal">> => {
    ...
    return err(internal(`The response did not match the ${contract} contract.`, { type: "invalid_response" }));
  };
