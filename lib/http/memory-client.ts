...
export function createMemoryClient(config: MemoryConfig): HttpClient {
  const request = async <T>(
    ...
    if (await sleep(delayFor(config.latencyMs ?? route?.latencyMs), options.signal) === "aborted") {
    ...
    if (!route) {
      return err(stamp(internal(`No fixture route for ${method} ${path}`, { type: UNSERVED_ROUTE })));
    }

    const answered = (await route.handle(
      {
        method, path, params: options.params, body: options.body,
        token: config.getAccessToken?.() ?? null, correlationId, requestId,
      },
