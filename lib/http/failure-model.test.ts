...
import {
  all, asFailure, because, canceled, chain, conflict, DOMAIN_KINDS, err, FAILURE_KINDS,
...
const getTarget = async (c: HttpClient, id: string): Promise<Result<Target, ReadFailure>> =>
  (await c.get<Target>(`/targets/${id}`)).mapErr(asReadFailure);

const listTargets = async (c: HttpClient, workspace: string): Promise<Result<Target[], ReadFailure>> =>
  (await c.get<Target[]>("/targets", { params: { workspace } })).mapErr(asReadFailure);

const findPrimaryTarget = async (
  c: HttpClient, workspace: string,
): Promise<Result<Target | null, TransportFailure>> =>
  optional(
    (await c.get<Target>(`/workspaces/${workspace}/primary-target`)).mapErr(asReadFailure),
    absentWhenType("no_primary_target"),
  );

const createTarget = async (c: HttpClient, input: NewTarget): Promise<Result<Target, CreateFailure>> => {
  ...
  return (await c.post<Target>("/targets", { body: input })).mapErr(asCreateFailure);
};

const targetPage = async (
  c: HttpClient, workspace: string, id: string,
): Promise<Result<{ target: Target; siblings: Target[] }, ReadFailure>> => {
  const [target, siblings] = await Promise.all([getTarget(c, id), listTargets(c, workspace)]);
...
const routes: MemoryRoute[] = [
  { method: "GET", pattern: /^\/targets$/, handle: (req) => { const t = requireToken(req); return t.ok ? ok([TARGET]) : t; } },
  { method: "GET", pattern: /^\/targets\/t1$/, handle: () => ok(TARGET) },
  { method: "GET", pattern: /^\/targets\/gone$/, handle: () => err(notFound("No such target", { status: 404 })) },
  { method: "GET", pattern: /^\/targets\/busy$/, handle: () => err(rateLimited("Slow down", 30, { status: 429 })) },
  { method: "GET", pattern: /^\/targets\/moved$/, handle: () => err(conflict("State moved", { status: 409 })) },
  { method: "GET", pattern: /^\/workspaces\/none\/primary-target$/, handle: () => err(notFound("none set", { status: 404, type: "no_primary_target" })) },
  { method: "GET", pattern: /^\/workspaces\/set\/primary-target$/, handle: () => ok(TARGET) },
  { method: "GET", pattern: /^\/workspaces\/typo\/primary-target$/, handle: () => err(notFound("Cannot GET /workspaces/typo/primary-targe", { status: 404 })) },
  { method: "GET", pattern: /^\/workspaces\/guarded\/primary-target$/, handle: (req) => { const t = requireToken(req); return t.ok ? ok(TARGET) : t; } },
];

const client = createMemoryClient({ routes, latencyMs: 0, getAccessToken: () => "tok" });
const anon = createMemoryClient({ routes, latencyMs: 0 });

describe("1 · the split", () => {
  it("partitions the ten kinds exactly", () => {
    expect(TRANSPORT_KINDS.length + DOMAIN_KINDS.length).toBe(FAILURE_KINDS.length);
  ...
  it("narrow passes every transport kind through untouched", () => {
    ...
    for (const f of [unauthenticated("a"), rateLimited("b", 30), canceled("c"), timeout("d")]) {
...
describe("3 · three states, with absence named by the service", () => {
  ...
  it("found is found", async () => {
    expect(presenceOf(await findPrimaryTarget(client, "set")).state).toBe("found");
  });
  ...
  it("v2's defect: an unserved fixture route is unmeasured, never empty", async () => {
    const r = await findPrimaryTarget(createMemoryClient({ routes: [], latencyMs: 0 }), "anything");
    if (r.ok) throw new Error("expected failure");
    expect(r.error.kind).toBe("internal");
  ...
  it("all three states are distinct", async () => {
    const states = (await Promise.all([
      findPrimaryTarget(client, "none"), findPrimaryTarget(client, "set"), findPrimaryTarget(anon, "guarded"),
    ])).map((r) => presenceOf(r).state);
    expect(new Set(states).size).toBe(3);
  });
  it("anyNotFound is the weak choice and does what it says", async () => {
    const r = await optional(err<Target, Failure>(notFound("whatever")), anyNotFound);
    expect(r.ok && r.value).toBeNull();
    const typed = await optional(err<Target, Failure>(notFound("whatever")), absentWhenType("no_primary_target"));
...
describe("4 · the port returns Result", () => {
  ...
  it("fields exist on invalid and are typed there", async () => {
    ...
    expect(r.error.fields).toEqual({ name: "A name is required.", host: "That is not a hostname." });
  });
  it("all preserves the tuple, returns the first failure, reads left to right", async () => {
    const page = await targetPage(client, "w1", "t1");
    expect(page.ok && page.value).toEqual({ target: TARGET, siblings: [TARGET] });
    const bad = all([ok(1), err<number>(timeout("a")), err<number>(internal("b"))]);
...
describe("5 · the fetch client and the envelope", () => {
  ...
  const http = (extra = {}) => createFetchClient({ baseUrl: "https://api.test/", ...extra });

  it("a problem document's kind wins over the status", async () => {
    respond(400, { kind: "conflict", message: "Version mismatch", request_id: "req-9" });
    const r = await http().get("/x");
    if (r.ok) throw new Error("expected failure");
    expect(r.error).toMatchObject({ kind: "conflict", message: "Version mismatch", requestId: "req-9", status: 400 });
  });
  it("without a kind, the status decides, and an RFC 9457 body still yields a message", async () => {
    respond(422, { title: "Unprocessable", detail: "host is malformed", errors: {} });
    const r = await http().get("/x");
    if (r.ok) throw new Error("expected failure");
  ...
  it("retry-after is read from the header when the body has none", async () => {
    ...
    const r = await http().get("/x");
    if (r.ok || r.error.kind !== "rate_limited") throw new Error("expected rate_limited");
  ...
  it("a custom decoder replaces the envelope and nothing else changes", async () => {
    ...
    const r = await http({
      decodeFailure: async (res: Response) => {
        ...
        return unauthenticated(body.error.msg, { type: body.error.code, status: res.status });
      },
    }).get("/x");
    if (r.ok) throw new Error("expected failure");
    expect(r.error).toMatchObject({ kind: "unauthenticated", type: "E_TEAPOT", status: 500 });
  });
  ...
  it("the caller's abort is `canceled`, matched by name so jsdom cannot break it", async () => {
    ...
    expect(failureFromTransport(new Error("ECONNREFUSED")).kind).toBe("unavailable");
  });
});
...
describe("6 · the one throw site", () => {
  it("unwrap throws an AppError and asFailure recovers it, totally", async () => {
    const r = await listTargets(anon, "w1");
    try { unwrap(r); throw new Error("unreachable"); }
    catch (e) { expect(asFailure(e).kind).toBe("unauthenticated"); }
  ...
  it("a Result serialised by accident degrades to plain data, not {}", () => {
    expect(JSON.parse(JSON.stringify(ok(TARGET)))).toEqual({ ok: true, value: TARGET });
  });
});
