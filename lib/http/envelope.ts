import {
  canceled, isFailureKind, timeout, unavailable,
  type Failure, type FailureKind, type FailureMeta,
} from "../kernel";
...
export const failureFromResponse: FailureDecoder = async (response) => {
  ...
  const meta: FailureMeta = {
    message:
      str(problem.message) ?? str(problem.detail) ?? str(problem.title) ??
      str(response.statusText) ?? "Request failed",
