...
export function withDiagnostics(inner: HttpClient): HttpClient {
  const request: HttpClient["request"] = <T>(method: string, path: string, options: RequestOptions = {}) =>
    options.trace ? options.trace.run("request", () => inner.request<T>(method, path, options)) : inner.request<T>(method, path, options);
  return {
    request,
