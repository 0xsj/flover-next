...
import { responseArray, responseDecoder, responseObject, responseText, responseTimestamp } from "./response";

describe("successful response decoding", () => {
  it("distinguishes explicit null from a rejected shape", () => {
    const decode = responseDecoder("optional record", value => value === null ? null : undefined);
  ...
  it("contains a throwing reader without leaking its exception", () => {
    const decode = responseDecoder("safe", () => { throw new Error("secret token"); });
  ...
  it("allows falsy, deliberately decoded values", () => {
    for (const value of [false, 0, ""]) expect(responseDecoder("value", () => value)(null).unwrapOr("wrong")).toBe(value);
  });

  it("validates every array member, including holes, and preserves order", () => {
    const read = responseArray(value => typeof value === "number" ? value : undefined);
  ...
  it("does not coerce records, text, or timestamps", () => {
    for (const value of [null, [], "{}", 0]) expect(responseObject(value)).toBeUndefined();
    for (const value of [null, true, 123, "", "  "]) expect(responseText(value)).toBe(false);
    for (const value of ["yesterday", "2026-01-01", "2026-99-01T00:00:00Z", 123]) expect(responseTimestamp(value)).toBe(false);
