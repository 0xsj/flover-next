/* Written behind an enforced barrier, from a contract alone.
 *
 * The author of this file had one tool — Write. No Read, no Bash, no search. It
 * never saw the implementation, so it could not pin what the code happens to do;
 * it could only pin what the contract said.
 *
 * Two edits were made afterwards, both mechanical, neither touching an
 * expectation: one type argument supplied where `err()` defaults its value type
 * to `never`, and one call reshaped where the port takes a body inside its
 * options object. Four tests are `it.skip` and each carries a TRIAGED comment
 * naming the gap in the CONTRACT that made it unwritable — none of them found a
 * defect in the code.
 *
 * Scored by mutation: 9 of 10, against 6 of 10 for the suite written alongside
 * the implementation. Evidence in custody/evidence/0001.
 *
 * Do not hand-edit this against the implementation. That is the one action that
 * silently converts it back into a snapshot. */
...
describe("2 · the ten kinds, and the split", () => {
  ...
  it("transport and domain kinds partition the ten kinds with no overlap", () => {
    const overlap = TRANSPORT_KINDS.filter((k) => (DOMAIN_KINDS as readonly string[]).includes(k));
...
describe("3 · constructors", () => {
  const plainConstructors: Array<[string, (message: string, meta?: any) => Failure, FailureKind]> = [
  ...
  describe.each(plainConstructors)("%s()", (_name, ctor, kind) => {
    ...
    it("applies optional meta (type, requestId, status, cause) verbatim", () => {
      ...
      const f = ctor("boom", { type: "custom", requestId: "req-1", status: 418, cause });
  ...
  describe("invalid()", () => {
    it("requires and carries the fields record", () => {
      ...
      expect(f.fields).toEqual({ email: "must be a valid email", age: "must be a number" });
    });
...
describe("4 · guards", () => {
  describe("isFailureKind", () => {
    ...
    it.each(["bogus", "", "NOT_FOUND", "notfound", 123, null, undefined, {}, []])(
      "is false for %p",
      (v) => {
  ...
  describe("isTransportKind / isDomainKind", () => {
    it("agree with TRANSPORT_KINDS / DOMAIN_KINDS membership for every kind", () => {
      for (const kind of FAILURE_KINDS) {
        expect(isTransportKind(kind)).toBe((TRANSPORT_KINDS as readonly string[]).includes(kind));
        expect(isDomainKind(kind)).toBe((DOMAIN_KINDS as readonly string[]).includes(kind));
      }
    });
  ...
  describe("isFailure", () => {
    ...
    it("is false for an object whose kind is not one of the ten", () => {
      expect(isFailure({ kind: "teapot", message: "I am a teapot" })).toBe(false);
    });
...
describe("5 · retry", () => {
  ...
  describe("retryDelay", () => {
    it("is null for every non-retryable kind", () => {
      for (const kind of FAILURE_KINDS) {
        if (kind === "rate_limited" || kind === "unavailable" || kind === "timeout") continue;
...
describe("7 · narrowing", () => {
  ...
  it("turns a domain failure outside the allowed list into internal, carrying the original as cause", () => {
    const original = conflict("stale", { type: "version_mismatch", requestId: "r-9", status: 409 });
    const mapper = narrow("not_found");
...
describe("8 · exhaustiveness", () => {
  ...
  it("surfaces the optional context in the thrown error's message", () => {
    expect(() => assertNever("unhandled" as never, "switch over failure.kind")).toThrow(
      /switch over failure\.kind/,
    );
  });
...
describe("9 · Result", () => {
  ...
  describe("match()", () => {
    ...
    it("calls exactly the err branch on failure and returns its value", () => {
      const okSpy = vi.fn();
      const failure = internal("boom");
      const value = err(failure).match({ ok: okSpy, err: (e: Failure) => e.message });
  ...
  describe("fromJSON()", () => {
    ...
    it("round-trips: fromJSON(x.toJSON()) behaves the same as x", () => {
      ...
      expect(fromJSON(originalOk.toJSON()).unwrapOr(-1)).toBe(originalOk.unwrapOr(-1));

      const originalErr = err(notFound("gone"));
      expect(fromJSON(originalErr.toJSON()).toJSON()).toEqual(originalErr.toJSON());
    });
  });
});
...
describe("10 · absence is not failure", () => {
  describe("optional()", () => {
    ...
    it("turns a not_found the predicate accepts as absence into a success of null", async () => {
      const result = await optional(err(notFound("no such thing")), anyNotFound);
  ...
  describe("presenceOf()", () => {
    it("is found for a success with a non-null value", () => {
      expect(presenceOf(ok({ id: 1 }))).toEqual({ state: "found", value: { id: 1 } });
    });
    ...
    it("is unmeasured for a failure — nobody looked, and that is a different fact from emptiness", () => {
      ...
      expect(presenceOf(err(failure))).toEqual({ state: "unmeasured", failure });
    });
  });
});
...
describe("12 · the transport port", () => {
  describe("decoding a non-2xx response — status-derived kind when there is no usable body", () => {
    ...
    it("decoding is total: a non-JSON body still produces a failure rather than throwing", async () => {
      vi.stubGlobal("fetch", async () => new Response("<html>Error</html>", { status: 500 }));
      try {
        const client = createFetchClient({ baseUrl: "https://api.test" });
        const result = await client.get("/resource");
        expect(result.ok).toBe(false);
    ...
    it("a body-declared kind that IS one of the ten wins over the status", () => {
      return (async () => {
        vi.stubGlobal(
          "fetch",
          async () =>
            new Response(JSON.stringify({ kind: "conflict", message: "stale data" }), {
              status: 400,
    ...
    it("a body-declared kind that is NOT one of the ten falls back to the status, preserving the unknown string as type", async () => {
      vi.stubGlobal(
        "fetch",
        async () =>
          new Response(JSON.stringify({ kind: "weird_backend_code", message: "oops" }), {
            status: 400,
            headers: { "content-type": "application/json" },
          }),
      );
    ...
    it("collects per-field messages for invalid, ignoring non-string entries", async () => {
      ...
      try {
        ...
        if (!result.ok) {
          ...
          expect((result.error as any).fields).toEqual({ name: "required", note: "ok" });
        }
      } finally {
        vi.unstubAllGlobals();
      }
    ...
    it.skip("takes requestId from the body when present", async () => {
      vi.stubGlobal(
        "fetch",
        async () =>
          new Response(JSON.stringify({ message: "gone", requestId: "body-rid" }), {
            status: 404,
            headers: { "content-type": "application/json" },
          }),
      );
      ...
      vi.stubGlobal(
        ...
        async () =>
          new Response(JSON.stringify({ message: "gone" }), {
            status: 404,
            headers: { "content-type": "application/json", "x-request-id": "hdr-rid" },
          }),
      );
      try {
        const client = createFetchClient({ baseUrl: "https://api.test" });
    ...
    it.skip("takes retryAfter from the body when present, for rate_limited", async () => {
      vi.stubGlobal(
        "fetch",
        async () =>
          new Response(JSON.stringify({ message: "slow down", retryAfter: 42 }), {
            status: 429,
            headers: { "content-type": "application/json" },
          }),
      );
    ...
    it("falls back to the retry-after header when the body has no retryAfter", async () => {
      ...
      try {
        ...
        const result = await client.get("/resource");
        expect(result.ok).toBe(false);
        if (!result.ok) expect(Number((result.error as any).retryAfter)).toBe(17);
      } finally {
        vi.unstubAllGlobals();
      }
    });
  ...
  describe("a failure with no response", () => {
    it("maps caller cancellation to canceled — distinct from and not retryable, unlike timeout", async () => {
      ...
      try {
        const client = createFetchClient({ baseUrl: "https://api.test" });
        const result = await client.get("/resource", { signal: controller.signal } as any);
        expect(result.ok).toBe(false);
        if (!result.ok) {
    ...
    it.skip("maps exceeding its own time budget to timeout — distinct from cancellation, and retryable", async () => {
      ...
      try {
        const client = createFetchClient({ baseUrl: "https://api.test", timeoutMs: 20 });
        const result = await client.get("/resource");
        expect(result.ok).toBe(false);
        if (!result.ok) {
    ...
    it("never throws, even when fetch itself throws synchronously", async () => {
      ...
      try {
        const client = createFetchClient({ baseUrl: "https://api.test" });
        await expect(client.get("/resource")).resolves.toMatchObject({ ok: false });
      } finally {
        vi.unstubAllGlobals();
      }
    });
  ...
  describe("the in-memory adapter", () => {
    it("dispatches a matching route and returns its Result untouched", async () => {
      ...
      expect(result).toEqual({ ok: true, value: { id: "abc", path: "/widgets/abc", method: "GET" } });
    });
    ...
    it("forwards the request body to the matching route's handler", async () => {
      ...
      const result = await client.post("/widgets", { body: { name: "gadget" } });
    ...
    it("a request whose signal is already aborted produces canceled", async () => {
      ...
      controller.abort();
      const client = createMemoryClient({
        routes: [{ method: "GET", pattern: /^\/x$/, handle: () => ok("should not matter") }],
      });
      const result = await client.get("/x", { signal: controller.signal } as any);
      expect(result.ok).toBe(false);
    ...
    it("forwards the resolved access token onto the request handed to a route", async () => {
      const client = createMemoryClient({
        routes: [{ method: "GET", pattern: /^\/me$/, handle: (request: any) => ok(request.token) }],
