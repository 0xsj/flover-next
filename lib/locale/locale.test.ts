...
describe("explicit presentation context (implementation-visible)", () => {
  it.each([{ locale: "bad_tag" }, { locale: "zz-ZZ" }, { locale: "" }, { timeZone: "Mars/Olympus" }, { timeZone: "" }, { currency: "usd" }, { currency: "" }])("rejects invalid context %s", patch => {
  ...
  it("formats number separators and minor units for the chosen locale/currency", () => {
    const de = createFormatters({ ...context, locale: "de-DE", currency: "EUR" }).unwrapOr(null)!;
    expect(de.number(1234.5).unwrapOr("")).toBe("1.234,5");
    expect(de.money(1234.5).unwrapOr("")).toContain("1.234,50");
    const jp = createFormatters({ ...context, currency: "JPY" }).unwrapOr(null)!;
  ...
  it("uses the chosen time zone across a calendar-day boundary and never the machine default", () => {
    ...
    const ny = createFormatters({ ...context, timeZone: "America/New_York" }).unwrapOr(null)!;
  ...
  it("distinguishes zero from invalid values and isolates a context from caller mutation", () => {
    const input = { ...context }, formatter = createFormatters(input).unwrapOr(null)!;
    input.timeZone = "Asia/Tokyo"; expect(formatter.context.timeZone).toBe("UTC");
    expect(formatter.number(0).unwrapOr("wrong")).toBe("0");
    for (const value of [NaN, Infinity, -Infinity]) { expect(formatter.number(value).ok).toBe(false); expect(formatter.money(value).ok).toBe(false); expect(formatter.instant(value).ok).toBe(false); }
