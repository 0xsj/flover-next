...
export function useLiveQueries<T>({ source, affected, resync, batchMs = 80 }: LiveQueryOptions<T>) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = source.subscribe({
      ...
      state(value) {
        setConnection(value);
        if (value.state === "open") { setFailure(null); schedule(resync); }
      },
      error: setFailure,
    });
    return () => { unsubscribe(); clearTimeout(timer); pending.clear(); };
  }, [cache, source, affected, resync, batchMs]);
