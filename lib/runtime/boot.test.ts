...
describe("the boot script agrees with the module path", () => {
  ...
  const cases = [
    ...
    { stored: { theme: "system" }, label: "system — the default, written back" },
    { stored: { density: "compact" }, label: "compact" },
    { stored: { theme: "dark", density: "compact" }, label: "both" },
    { stored: { theme: "sepia", density: "roomy" }, label: "values from an older build" },
  ];
...
describe("the boot script survives what storage does", () => {
  ...
  it("does not throw when storage throws", () => {
    ...
    Object.defineProperty(globalThis, "localStorage", {
      configurable: true,
      get() { throw new Error("denied by the browser"); },
    });
    try {
