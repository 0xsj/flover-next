...
describe("latest read ownership", () => {
  it.each([[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]])("the third invocation wins for completion order %j", async (a, b, c) => {
    const requests = [0, 1, 2].map(() => deferred<Result<number>>());
    const signals: AbortSignal[] = [];
    const reader = createLatestRead((index: number, signal) => { signals.push(signal); return requests[index].promise; });
    const pending = [0, 1, 2].map(index => reader.run(index));
    for (const index of [a, b, c]) { requests[index].resolve(ok(index)); await pending[index]; }
  ...
  it("ignores an obsolete failure after the newer request succeeds", async () => {
    ...
    const reader = createLatestRead((old: boolean) => old ? first.promise : Promise.resolve(ok("newer")));
  ...
  it("keeps failed, empty, and unmeasured reads distinct and recovers", async () => {
    const reader = createLatestRead((result: Result<string[]>) => Promise.resolve(result));
    expect(reader.get()).toEqual({ state: "idle" });
    await reader.run(err(internal("bad")));
    expect(reader.get()).toMatchObject({ state: "failed", previous: undefined });
    await reader.run(ok(["accepted"]));
    await reader.run(err(internal("bad refresh")));
    expect(reader.get()).toMatchObject({ state: "failed", previous: ["accepted"] });
    await reader.run(ok([]));
  ...
  it("cancels on cleanup, ignores late results, and can reconnect after cleanup", async () => {
    ...
    const reader = createLatestRead((old: boolean) => old ? late.promise : Promise.resolve(ok("current")));
    const pending = reader.run(true);
    reader.cancel();
