 ...
 */
export function createLatestRead<Input, Value>(read: (input: Input, signal: AbortSignal) => Promise<Result<Value>>) {
  ...
  return {
    ...
    async run(input: Input) {
      ...
      try { result = await read(input, signal); }
    ...
    cancel() { generation++; controller?.abort(); store.set(settled); },
  };
}
