...
const schema = querySchema({ q: textParam({ maxLength: 20 }), page: integerParam({ max: 100 }), view: choiceParam(["table", "cards"], "table") });

describe("typed query state", () => {
  it("infers field types and gives missing fields explicit defaults without issues", () => {
    expectTypeOf(schema.defaults).toEqualTypeOf<{ readonly q: string; readonly page: number; readonly view: "table" | "cards" }>();
    expect(schema.read("unrelated=1")).toEqual({ value: { q: "", page: 1, view: "table" }, issues: [] });
  });
  ...
  it.each(["0", "-1", "+2", " 2 ", "2.5", "1e2", "2bad", "Infinity", "101", "9007199254740992", ""])("rejects invalid integer %j", raw => {
  ...
  it("rejects duplicate scalars, even when both values agree", () => {
    ...
    expect(schema.read("q=one&q=two&view=invalid").issues.map(issue => issue.key)).toEqual(["q", "view"]);
  });

  it("trims and bounds text and matches choices exactly", () => {
    expect(schema.read("q=%20hello%20&view=cards").value).toEqual({ q: "hello", page: 1, view: "cards" });
    expect(schema.read(new URLSearchParams({ q: "a".repeat(21), view: "CARDS" })).issues).toHaveLength(2);
  });

  it("omits defaults while preserving unrelated repeated keys without mutating the base", () => {
    const base = new URLSearchParams("keep=a&keep=b&page=9&view=cards&chaosSeed=7");
    const before = base.toString();
    const result = schema.write({ q: "", page: 1, view: "table" }, base);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.toString()).toBe("keep=a&keep=b&chaosSeed=7");
    expect(base.toString()).toBe(before);
  });

  it("makes canonical writes idempotent and round-trips Unicode and special characters", () => {
    for (const q of ["", "hello world", "İstanbul", "雪", "a&b=+?#", "  trim me  "]) {
      for (const page of [1, 2, 100]) {
        const result = schema.write({ q, page, view: "cards" }, "keep=a&keep=b");
        if (!result.ok) throw new Error("expected a valid state");
        const read = schema.read(result.value);
        expect(read).toEqual({ value: { q: q.trim(), page, view: "cards" }, issues: [] });
        const again = schema.write(read.value, result.value);
        expect(again.ok && again.value.toString()).toBe(result.value.toString());
      }
    }
  });
  ...
  it("contains a failing updater", () => {
    expect(schema.update("", () => { throw new Error("PRIVATE"); }).ok).toBe(false);
  });
  ...
  it("requires valid defaults and treats special object keys as ordinary owned keys", () => {
    ...
    const unusual = querySchema(Object.fromEntries([["__proto__", textParam()]]));
    expect(unusual.read("__proto__=text").value.__proto__).toBe("text");
    expect(Object.getPrototypeOf(unusual.read("__proto__=text").value)).toBeNull();
  });
});
